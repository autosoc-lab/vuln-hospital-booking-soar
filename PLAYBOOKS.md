...
> SSRF -> SSE-C 랜섬웨어 체인용으로 이미 설계됐던 2단계 AWS 자동조치(100031/100032
> lifecycle 원복, 100014 세션 revoke)는 `playbook_ex.md`에 보존되어 있다. 이 저장소의
> Terraform 코드(`shuffle_lifecycle_revert`/`shuffle_revoke_session`)에는 더 이상
> 반영돼 있지 않으므로, 다시 붙일지 여부는 별도로 논의 후 결정할 것.
