...
clone -> `.env` 생성 -> `docker compose up -d`까지 자동으로 처리한다 (완료까지 3~5분
정도 걸림 — opensearch가 뜨는 데 시간이 좀 걸린다).
...
`vuln-hospital-booking-infra`의 `modules/wazuh`가 이미 다음을 EC2에 배포해둔 상태다
(이 레포와는 별개로, 인프라 쪽 terraform apply만으로 설치됨):
...
Shuffle 웹훅 URL은 워크플로를 만들어야 나오는 값이라 terraform이 자동으로 넣어주지
않는다 — Shuffle 배포 후 [PLAYBOOKS.md](PLAYBOOKS.md)의 3~4번 단계를 따라 수동으로 한 번
연결해야 한다:

```bash
ssh -i <infra repo>/ssh/vuln-hospital-lab.pem ubuntu@<wazuh_manager_public_ip>
...
## 현재 범위: 알림/케이스 생성까지 (자동 조치 없음)

이 초기 버전은 실제 AWS 리소스를 변경하는 조치(IAM 키 비활성화, 보안그룹 격리 등)를
하지 않는다 — Shuffle EC2에는 AWS 쓰기 권한을 아예 주지 않았다(SSM 접속용 권한만).
Slack/이메일 알림과 케이스 생성까지 검증한 뒤, PLAYBOOKS.md 6번 섹션을 따라 필요한
자동조치를 단계적으로 추가하는 걸 권장한다.
